...
int handle_client_input(char *buffer)
{
  ...
  else if (!strcmp(command, "start_duel"))
  {
    ...
    strip_newline(it); // Nettoie le saut de ligne dans l'argument
    strcpy(buffer, "/001 ");
  ...
  else if (!strcmp(command, "private_message"))
  {
    ...
    message++;              // Passe l'espace après le nom d'utilisateur
    strip_newline(message); // Nettoie le saut de ligne dans le message
