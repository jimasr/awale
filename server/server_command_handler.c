...
void send_ranking_to_client(Client *client) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (strlen(line) < 2) {
            printf("Skipped empty or malformed line.\n");
            continue;
        }
        ...
        if (player1 == NULL || player2 == NULL) {
            printf("Malformed game entry, skipping.\n");
            continue; // Ignorez les entrées mal formées
        }
...
void start_watching_player(ActiveClients clients, Client *client, char *username,
                           char *buffer)
{
  ...
  else if (client_to_watch->priv && !are_clients_friends(client, client_to_watch))
  {
    strcpy(buffer, username);
  ...
  else
  {
    ...
    observer->next = NULL;
    add_observer_to_list(client_to_watch->observers, observer);
